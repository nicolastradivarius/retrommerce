...
function splitLocaleFromPath(pathname: string): { locale: Locale | null; basePath: string } {
	// Matches: "/es" or "/es/..."
	...
	if (!isSupportedLocale(detectedLocale)) return { locale: null, basePath: pathname };

	const remainingPath = match[2] || '';
	return { locale: detectedLocale, basePath: remainingPath === '' ? '/' : remainingPath };
}
...
const redirectToRoleHome = (role: string, request: NextRequest, locale: Locale | null) => {
...
export function proxy(request: NextRequest) {
	...
	if (!localeInPath) {
		...
		// Root should always land on /products
		if (pathname === '/') {
	...
	// Locale root should always land on /{locale}/products
	if (basePath === '/') {
		const url = request.nextUrl.clone();
		url.pathname = `/${localeInPath}/products`;
		return NextResponse.redirect(url);
	}

	// Auth routing: only guard specific sections (basePath is without locale).
	const isProtected = basePath.startsWith('/admin') || basePath.startsWith('/user');
	...
	if (basePath.startsWith('/admin')) {
		if (payload.role !== 'ADMIN') return redirectToRoleHome(payload.role ?? '', request, localeInPath);
	}

	// Guard user routes: only USER users are allowed.
	if (basePath.startsWith('/user')) {
		if (payload.role !== 'USER') return redirectToRoleHome(payload.role ?? '', request, localeInPath);
	}
